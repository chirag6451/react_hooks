...
try {
  ...
  if (!packageJson.scripts['check-gitignore']) {
    ...
    if (!fs.existsSync(checkGitignorePath)) {
      ...
      const checkGitignoreContent = `#!/usr/bin/env node

/**
 * Script to check and update .gitignore file
 */
...
`;
  ...
  if (!packageJson.scripts['build:dev']) {
    ...
    if (!fs.existsSync(buildReactAppsPath)) {
      ...
      const buildReactAppsContent = `#!/usr/bin/env node

/**
 * Script to build all React apps in the repository
 */
